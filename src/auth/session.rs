...
pub async fn validate_session(token: String, pool: &Pool<AsyncMysqlConnection>) -> Result<Session, (StatusCode, String)> {
    ...
    let session: Session = sessions::table.select(sessions::all_columns).filter(sessions::id.eq(&session_id)).first(&mut conn).await.map_err(|_| (StatusCode::UNAUTHORIZED, String::from("401 Unauthorized")))?;
