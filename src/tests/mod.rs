...
async fn login_wrong_password() {
    ...
    let response = srv.oneshot(
        Request::builder()
            ...
            .uri("/sign-in")
            .header("content-type", "application/x-www-form-urlencoded")
            .body(Body::from("email=testemail@securecart.com&password=mywrongpassword"))
            .unwrap()
    ).await.unwrap();
    assert_eq!(response.status(), StatusCode::UNAUTHORIZED)
}
...
async fn login_correct() {
    ...
    let response = srv.oneshot(
        Request::builder()
            ...
            .uri("/sign-in")
            .header("content-type", "application/x-www-form-urlencoded")
            .body(Body::from("email=testemail@securecart.com&password=mysecurepassword"))
            .unwrap()
    ).await.unwrap();
    assert_eq!(response.status(), StatusCode::OK)
}
